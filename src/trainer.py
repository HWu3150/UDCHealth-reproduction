...
from metrics import multilabel_metrics_fn

from pyhealth.utils import create_directory
from config import config
from utils import plot_tsne, adjust_learning_rate
...
class Trainer:
    ...
    def inference(self, dataloader, additional_outputs=None,
        ...
        for data in tqdm(dataloader, desc="Evaluation"):
            self.model.eval()
            with torch.no_grad():
                output = self.model(train_mode='finetune', **data) # 永远只看fine_tune，不然最好改成不同的trainer，这样可以指定多个metric
                loss = output["loss"]
...
# def save_data(model, y_prob, label):
